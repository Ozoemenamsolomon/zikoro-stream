...
import { TOrganizerStream, TStreamAttendee } from "@/types";
...
function VerifyEmailWidget({
  ...
  const { postData: addAttendee } =
    usePostRequest<Partial<TStreamAttendee>>("/stream/attendee");
...
export default function StreamAttendeeRegsitration({
  streamAlias,
}: {
  streamAlias: string;
}) {
